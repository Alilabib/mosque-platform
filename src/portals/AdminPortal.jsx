...
// --- Initial Data ---
const initMosques=()=>[
  {id:1,name:"جامع الراجحي",city:"الرياض",district:"حي النسيم",type:"جامع",status:"نشط",capacity:3500,currentOccupancy:1247,services:["الجمعة","الجنائز","العيد","التحفيظ"],imam:"الشيخ عبدالله المحمد",device:"متصل"},
  {id:2,name:"مسجد الفرقان",city:"الرياض",district:"حي الملز",type:"مسجد",status:"نشط",capacity:800,currentOccupancy:312,services:["الجمعة","التحفيظ"],imam:"الشيخ خالد العتيبي",device:"متصل"},
  {id:3,name:"جامع الملك فهد",city:"جدة",district:"حي الحمراء",type:"جامع",status:"نشط",capacity:5000,currentOccupancy:2834,services:["الجمعة","الجنائز","العيد","التحفيظ","المحاضرات"],imam:"الشيخ سعد الغامدي",device:"غير متصل"},
  {id:4,name:"مسجد التوحيد",city:"الدمام",district:"حي الفيصلية",type:"مسجد",status:"صيانة",capacity:600,currentOccupancy:0,services:["الجمعة"],imam:"الشيخ محمد الدوسري",device:"صيانة"},
  {id:5,name:"مسجد الإيمان",city:"مكة المكرمة",district:"حي العزيزية",type:"مسجد",status:"نشط",capacity:1200,currentOccupancy:456,services:["الجمعة","التحفيظ","المحاضرات"],imam:"الشيخ أحمد الشهري",device:"متصل"},
  {id:6,name:"جامع البواردي",city:"الرياض",district:"حي العليا",type:"جامع",status:"نشط",capacity:2800,currentOccupancy:891,services:["الجمعة","الجنائز","العيد"],imam:"الشيخ فهد القحطاني",device:"متصل"},
  {id:7,name:"مسجد النور",city:"المدينة المنورة",district:"حي قباء",type:"مسجد",status:"نشط",capacity:950,currentOccupancy:523,services:["الجمعة","التحفيظ"],imam:"الشيخ ياسر الحربي",device:"متصل"},
  {id:8,name:"مصلى حي الروضة",city:"الرياض",district:"حي الروضة",type:"مصلى",status:"مغلق مؤقتاً",capacity:200,currentOccupancy:0,services:[],imam:"—",device:"غير متصل"},
];
const initKhutbahs=()=>[
...
function Dashboard({mosques,tickets,complaints,khutbahs,donations}){
  ...
  return<div style={{display:"flex",flexDirection:"column",gap:20}}>
    <div className="admin-stat-grid" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
      ...
      <Stat icon="👥" label="المتواجدون الآن" value={mosques.reduce((s,m)=>s+m.currentOccupancy,0).toLocaleString()} sub="مصلي" bg="#f0fdf4"/>
    </div>
...
function MosquesPage({mosques,setMosques,toast}){
  ...
  if(sel){return<div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {detailTab==="info"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}><Card title="بيانات المسجد"><div style={{display:"grid",gridTemplateColumns:"100px 1fr",gap:"10px 14px",fontSize:13.5}}>{[["النوع",sel.type],["المدينة",sel.city],["الحي",sel.district],["السعة",`${sel.capacity} مصلي`],["المتواجدون الآن",`${sel.currentOccupancy} مصلي (${Math.round(sel.currentOccupancy/sel.capacity*100)}%)`],["الإمام",sel.imam],["الحالة",sel.status]].map(([l,v],i)=><div key={i} style={{ display: "contents" }}><span style={{color:C.text2,fontWeight:600}}>{l}</span><span>{v}</span></div>)}</div></Card><Card title="إحصائيات"><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={{textAlign:"center",padding:14,background:C.primaryLight,borderRadius:10}}><div style={{fontSize:24,fontWeight:700}}>{sel.services.length}</div><div style={{fontSize:12,color:C.text2}}>خدمات</div></div><div style={{textAlign:"center",padding:14,background:C.infoLight,borderRadius:10}}><div style={{fontSize:24,fontWeight:700}}>{sel.capacity}</div><div style={{fontSize:12,color:C.text2}}>السعة</div></div></div><div style={{display:"grid",gridTemplateColumns:"1fr",gap:10,marginTop:10}}><div style={{padding:14,background:"#f0fdf4",borderRadius:10,border:"1px solid #bbf7d0"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div><div style={{fontSize:11,color:"#4ade80",marginBottom:4}}>المتواجدون الآن</div><div style={{display:"flex",alignItems:"baseline",gap:4}}><span style={{fontSize:28,fontWeight:700,color:"#15803d"}}>{sel.currentOccupancy}</span><span style={{fontSize:12,color:"#86efac"}}>/ {sel.capacity}</span></div></div><div style={{fontSize:11,color:C.text3,textAlign:"center"}}><div>📷</div><div>كاميرا ذكية</div></div></div><div style={{height:6,background:"#dcfce7",borderRadius:3,marginTop:10,overflow:"hidden"}}><div style={{width:`${Math.round(sel.currentOccupancy/sel.capacity*100)}%`,height:"100%",background:sel.currentOccupancy/sel.capacity>0.8?"#f97316":"#22c55e",borderRadius:3}}/></div></div></div></Card></div>}
  ...
  return<div style={{display:"flex",flexDirection:"column",gap:16}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><h2 style={{margin:0,fontSize:20,fontWeight:700}}>إدارة المساجد</h2><div style={{display:"flex",gap:8}}><Btn small variant="ghost" onClick={()=>exportCSV([{label:"الاسم",csvKey:"name"},{label:"النوع",csvKey:"type"},{label:"المدينة",csvKey:"city"},{label:"الحي",csvKey:"district"},{label:"السعة",csvKey:"capacity"},{label:"المتواجدون",csvKey:"currentOccupancy"},{label:"الإمام",csvKey:"imam"},{label:"الحالة",csvKey:"status"}],mosques,"mosques")}>تصدير CSV</Btn><Btn icon="+" onClick={()=>setModal(true)}>إضافة مسجد</Btn></div></div>
    ...
    <Card title="قائمة المساجد" action={<SearchBar value={search} onChange={setSearch} placeholder="بحث بالاسم أو المدينة..."/>} noPad>
      <Table cols={[{label:"المسجد",render:r=><span style={{fontWeight:600}}>{r.name}</span>},{label:"النوع",key:"type"},{label:"المدينة",key:"city"},{label:"الحي",key:"district"},{label:"السعة",key:"capacity"},{label:"المتواجدون",render:r=><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{width:6,height:6,borderRadius:"50%",background:r.currentOccupancy>0?"#22c55e":C.text3,display:"inline-block"}}/><span style={{fontWeight:600}}>{r.currentOccupancy}</span><span style={{fontSize:11,color:C.text3}}>/{r.capacity}</span></div>},{label:"الإمام",key:"imam"},{label:"الحالة",render:r=><Badge text={r.status} color={stC(r.status)}/>},{label:"الجهاز",render:r=><Badge text={r.device} color={dvC(r.device)}/>}]} rows={filtered} onRow={setSel}/>
    </Card>
